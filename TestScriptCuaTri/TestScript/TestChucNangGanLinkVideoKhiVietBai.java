...
public class TestChucNangGanLinkVideoKhiVietBai {
	 	WebDriver driver;
	  	...
	  	public void TestCase() throws InterruptedException, AWTException {
	  		...
	  		Thread.sleep(1000);
	  		driver.findElement(By.xpath("//div[contains(@class, 'title mb-4 cursor-text')]")).sendKeys("Kiểm thử tự động");
	  		Thread.sleep(1000);
	  		...
	  		Thread.sleep(2000);
	  		driver.findElement(By.xpath("//li[@class='ce-toolbox__button' and @data-tool='linkTool']")).click();
	  		Thread.sleep(2000);
	  		driver.findElement(By.xpath("//div[contains(@class, 'cdx-input') and contains(@class, 'link-tool__input')]")).sendKeys("https://www.youtube.com/watch?v=8ZvUQb_w-Qc");
	  		Thread.sleep(5000);	
	        ...
	        List<WebElement> videoThumbnail = driver.findElements(By.xpath("//div[contains(@class, 'link-tool__image')]"));

	        String expected = "Hình ảnh và tiêu đề video được hiển thị";
	        String actual = (videoThumbnail.size() > 0 && videoThumbnail.get(0).isDisplayed()) 
	                     ? "Hình ảnh và tiêu đề video được hiển thị" 
	                     : "Hình ảnh video KHÔNG được hiển thị";
	        System.out.println("Kết quả mong đợi: " + expected);
	     	System.out.println("Kết quả thực tế: " + actual);
	     	Assert.assertEquals(actual, expected, "Kết quả không như mong đợi!");
